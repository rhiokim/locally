...
program
   ...
   .option('-a, --cors <domain>', 'CORS(Cross-Origin Resource Sharing) enable');
...
function createServer() {
	...
	if (program.cors) {
		app.use(function(req, res, next) {
			res.setHeader('Access-Control-Allow-Origin', program.cors);
			next();
		});
	}

	app.use(connect.static(_static));
    ...
    app.use(function(req, res, next) {
        ...
        fs.readFile(_static, function(err){
        	if (err) {
        		if(err.errno === 28){
        			var uri = require('url').parse(req.url).pathname;
        			var path = _static + uri;
